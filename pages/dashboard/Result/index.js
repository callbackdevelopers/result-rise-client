...
const index = () => {
  ...
  const { data: resultData = [], refetch, isLoading } = useQuery({
      queryKey: [],
      queryFn: async () => {
          const res = await fetch(url)
          const data1 = await res.json()
          const data = data1.filter(result => result.student_email ===user.email)
          return data;
      }
  })
          ...
          <div className=" gap-3 p-4">
            <div>
              {resultData?.map((semester) => (
                <div className=" grid lg:grid-cols-2 gap-3" key={semester._id}>
                  {semester.semester_results.map((finaldata) => (
                    <div key={finaldata._id}>
                      <Link
                        href={`/dashboard/result/${finaldata.semesterId}`}
                      >
                        <SecondaryButton> {finaldata.semester}</SecondaryButton>
                      </Link>
                    </div>
                  ))}
